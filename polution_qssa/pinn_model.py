...
class PINN_Model(nn.Module):
    def __init__(self, nodes=40, layers=2, y0=0, w_scale=None, x_scale=1, use_softmax=True):
        ...
        # whether to ues softmax to constrain the output to (0,1)
    ...
    def forward(self, x):
        ...
        if self.use_softmax:
            ...
            y = self.y0 + self.w_scale * x_rescale * self.seq(torch.log(x_rescale))
            y_exp = torch.exp(y/scale)
            y = y_exp / y_exp.sum(1).view(-1,1)
